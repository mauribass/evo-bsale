...
if not EVO_USER or not EVO_PASS or not BSALE_TOKEN:
    raise RuntimeError("Faltan credenciales en variables de entorno (EVO_USER/EVO_PASS/BSALE_TOKEN).")

# Tipos de documento (obligatorios). Configúralos en Render.
DOCUMENT_TYPE_ID_NOM = int(os.getenv("DOCUMENT_TYPE_ID_NOM", "0"))  # ej: Boleta nominativa
DOCUMENT_TYPE_ID_NN  = int(os.getenv("DOCUMENT_TYPE_ID_NN",  "0"))  # ej: Boleta no nominativa (si aplica)

if DOCUMENT_TYPE_ID_NOM <= 0 or DOCUMENT_TYPE_ID_NN <= 0:
    raise RuntimeError(
        "Faltan DOCUMENT_TYPE_ID_NOM / DOCUMENT_TYPE_ID_NN en variables de entorno. "
        "Usa /debug/bsale para listar document types y configura los IDs correctos."
    )

PRICE_LIST_ID = int(os.getenv("PRICE_LIST_ID", "2"))
VARIANT_MAP_FILE = "variant_map.json"
VARIANT_ID_OTHERS = int(os.getenv("VARIANT_ID_OTHERS", "1244"))  # Reemplazar por el ID real en Bsale si corresponde

# Sucursales (ajústalas a tu cuenta)
...
def obtener_receivables(id_branch, inicio, fin):
    ...
    while True:
        ...
        # FIX: antes decía len(si); debe ser len(lote)
        if len(lote) < 50:
            break
        skip += 50
    return resultados
...
def emitir_boleta_bsale(data):
    ...
    if res.status_code not in [200, 201]:
        try:
            ...
            error_msg = res.text
        logger.error(f"BSale error {res.status_code}: {error_msg}")
...
# ============================================
# DEBUG: listar tipos de documento en Bsale
# ============================================
@app.route("/debug/bsale")
def debug_bsale():
    headers = {"access_token": BSALE_TOKEN}
    try:
        r = session.get("https://api.bsale.io/v1/document_types.json?limit=200", headers=headers, timeout=20)
        r.raise_for_status()
        items = r.json().get("items", [])
        filas = [
            f"{it.get('id')} — {it.get('name')} | printable={it.get('isPrintable')} | electronic={it.get('isElectronic')}"
            for it in items
        ]
        return "<pre>" + "\n".join(filas) + "</pre>"
    except Exception as e:
        logger.error(f"No se pudieron listar document_types: {e}")
